...
def compute_length_penalty(sequence_lengths: torch.Tensor, length_penalty_coef: float = 0.01) -> torch.Tensor:
    """
    计算长度惩罚：scores -= length_penalty_coef * length
    
    Args:
        sequence_lengths: 每个序列的实际长度 [batch_size]
        length_penalty_coef: 长度惩罚系数，默认 0.01
        
    Returns:
        长度惩罚值 [batch_size]，应该从 scores 中减去
    """
    if length_penalty_coef == 0.0:
        return torch.zeros_like(sequence_lengths, dtype=torch.float32)
    return length_penalty_coef * sequence_lengths.float()
...
def compute_repetition_penalty(
    ...
    pad_token_id: int,
    n_gram: int = 3,
    repetition_penalty_coef: float = 0.1,
) -> torch.Tensor:
    """
    计算 n-gram 重复惩罚
    
    Args:
        responses: 响应序列 [batch_size, seq_len]
        sequence_lengths: 每个序列的实际长度 [batch_size]
        pad_token_id: padding token ID
        n_gram: n-gram 的大小，默认 3
        repetition_penalty_coef: 重复惩罚系数，默认 0.1
        
    Returns:
        重复惩罚值 [batch_size]，应该从 scores 中减去
    """
    if repetition_penalty_coef == 0.0:
        return torch.zeros(responses.shape[0], dtype=torch.float32, device=responses.device)
    
    batch_size = responses.shape[0]
    penalties = torch.zeros(batch_size, dtype=torch.float32, device=responses.device)
    
    # 将数据移到 CPU 进行计算（n-gram 计算在 CPU 上更高效）
    responses_cpu = responses.cpu()
    sequence_lengths_cpu = sequence_lengths.cpu()
    
    for i in range(batch_size):
        seq_len = sequence_lengths_cpu[i].item()
        if seq_len < n_gram:
            # 序列太短，无法计算 n-gram，无惩罚
            continue
        
        # 提取实际序列（去除 padding）
        sequence = responses_cpu[i, :seq_len].numpy()
        
        # 计算 n-gram
        ngrams = {}
        total_ngrams = 0
        for j in range(len(sequence) - n_gram + 1):
        ...
        # 计算重复率：重复的 n-gram 数量 / 总 n-gram 数量
        # 重复的 n-gram 是指出现次数 > 1 的 n-gram
        repeated_ngrams = sum(1 for count in ngrams.values() if count > 1)
        repetition_ratio = repeated_ngrams / total_ngrams if total_ngrams > 0 else 0.0
        
        # 惩罚 = 系数 * 重复率
        penalties[i] = repetition_penalty_coef * repetition_ratio
    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                # Reward Shaping: 应用长度惩罚和重复惩罚来防止 reward hacking
                # 从 args 中获取惩罚系数，如果不存在则使用默认值
                ...
                repetition_n_gram = getattr(self.args, 'repetition_n_gram', 3)
                
                # 应用长度惩罚: scores -= length_penalty_coef * length
                if length_penalty_coef > 0:
                    length_penalties = compute_length_penalty(sequence_lengths, length_penalty_coef)
                    scores = scores - length_penalties.to(scores.device)
                
                # 应用重复惩罚: 基于 n-gram 重复率
                if repetition_penalty_coef > 0:
                    repetition_penalties = compute_repetition_penalty(
                        postprocessed_responses,
                        sequence_lengths,
                        processing_class.pad_token_id,
                        n_gram=repetition_n_gram,
                        repetition_penalty_coef=repetition_penalty_coef,
                    )
                    scores = scores - repetition_penalties.to(scores.device)
